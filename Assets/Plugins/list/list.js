$(document).ready(function(){
	...
	//定义当前、之前、之后要显示图片的排位
	var curPic=0,nextPic=-1,prePic=-1,viewPic=4;preShowPic=-1;
	...
	function smallPicScroll(){
		if(curPic != preShowPic) {
			...
			if(curPic>2 && curPic<($(".scrolltab .ulSmallPic li").length-3)) {
				leftPosition=-(curPic-2)*98;
			}else if(curPic > ($(".scrolltab .ulSmallPic li").length-4) && $(".scrolltab .ulSmallPic li").length>viewPic) {
				leftPosition=-($(".scrolltab .ulSmallPic li").length-viewPic)*98;
			}
			leftPosition+="px";
	...
	$("#sLeftBtnB").click(function(){
		if($(this).attr("class")=="sLeftBtnBSel") {
			...
			leftPosition=leftPositionNum+98+"px";
			if(leftPosition=="0px") {if($(this).attr("class") != "sLeftBtnBBan") {$(this).attr("class","sLeftBtnBBan");}}
			var bestLeftNum=-($(".scrolltab .ulSmallPic li").length-viewPic)*98;
			if((leftPositionNum+98) > bestLeftNum && $("sRightBtnB").attr("class") != "sRightBtnB") {$("#sRightBtnB").attr("class","sRightBtnB")}
	...
	$("#sRightBtnB").click(function(){
		if($(this).attr("class")=="sRightBtnBSel"){
			...
			leftPosition=leftPositionNum-98+"px";
			var bestLeftNum=-($(".scrolltab .ulSmallPic li").length-viewPic)*98;
			if((leftPositionNum-98)==bestLeftNum) {$(this).attr("class","sRightBtnBBan");}
